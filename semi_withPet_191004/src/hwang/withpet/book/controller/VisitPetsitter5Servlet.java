...
/**
 * Servlet implementation class VisitPetsitter5Servlet
 */
@WebServlet(name ="visitPetsitter5", urlPatterns="/book/visitPetsitter5")
public class VisitPetsitter5Servlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public VisitPetsitter5Servlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("\n" + time1 + " visitPetsitter5Sevlet 들어옴");

		///////////////
		HttpSession session = request.getSession(false);
		
		String viewPath = "/views/book/bookResult.jsp"; // view 페이지 경로
		...
		if(session!=null && session.getAttribute("loginMember")!=null) { // 로그인을 했으면
			...
			if (cookies != null) {
				...
				for (int i = 0; i < cookies.length; i++) {
					...
					} else if (cookieName.equals("petsitterId")) { // detailAddress 쿠키가 있으면
						petsitterId = cookieValue;
					}
				}
				...
				if (userPostcodeC.equals("")) {
					...
					Price price=new BookService().selectPrice();
					Petsitter petsitter=new BookService().selectPetsitterOne(petsitterId);
